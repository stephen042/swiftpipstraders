            ...
            <div class="page-content">
                ...
                if ($account_data["account_role"] == "Manager") {
                    if (isset($_GET["trade_id"]) && !empty($_GET["trade_id"])) {
                        ...
                        $stmt = $db_conn->prepare("SELECT * FROM `trades` WHERE id = ?");
                ...
                ?>
                            <script>
                                window.alert('The provided Trade ID is invalid!');
                                var managerOverview = 'authT.php';
                                window.location.href = managerOverview;
                            </script>
                        <?php
                        } else {
                            $row = mysqli_fetch_assoc($result);
                            $investorEmail = $row["userEmail"];

                            $stmt = $db_conn->prepare("SELECT * FROM `accounts` WHERE JSON_EXTRACT(`datasource`, '$.email_address') = ?");
                            $stmt->bind_param("s", $investorEmail);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $investorRow = mysqli_fetch_assoc($result);

                            $investor_data = json_decode($investorRow["datasource"], true);

                            //  Include manager area module
                            include "./modules/_data_source/manager_editTrade.php";
                        }
                    } else {
                        ?>
                        <script>
